...
// every rescan. The analysis lives only in the library (files are never
// tagged), so carry it over; a changed file (or a cue edit that moved the
// segment's start or end) is measured afresh.
const ANALYSIS_KEYS = ['loudnessLufs', 'loudnessRange', 'truePeakDb', 'replayGainTrack',
  'replayGainAlbum', 'rgTrackPeak', 'rgAlbumPeak', 'loudnessAnalyzedAt'];
function keepFromPrev(prev, fresh) {
  if (!prev || prev.mtime !== fresh.mtime) return fresh;
  if (prev.added) fresh.added = prev.added;
  if (prev.loudnessAnalyzedAt && (prev.cueStart ?? null) === (fresh.cueStart ?? null) &&
      (prev.cueEnd ?? null) === (fresh.cueEnd ?? null)) {
